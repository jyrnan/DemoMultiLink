...
struct UdpClientView: View {
    ...
    private var ipPicker: some View {
    ...
    private var searchAndConnetctButton: some View {
        HStack {
            Button {
    ...
    var body: some View {
        VStack(alignment: .leading) {
            ipPicker
            portInputField
            searchAndConnetctButton
            messageInputField
            sendButton

            Divider()
            Text("Log Message")
            TextEditor(text: $vm.logMessage).border(.gray.opacity(0.2))
        }
        .padding()
